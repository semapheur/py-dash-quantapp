...
from sklearn.preprocessing import (
  StandardScaler,
)  # StandardScaler, MinMaxScaler, MaxAbsScaler and RobustScaler
from sklearn import (
  metrics,
  preprocessing,
  neighbors,
...
def fuzzMatch(left, right):
  ...
  for i, l in enumerate(left):
    if l in right:
      ...
      x = process.extractOne(l, right, scorer=fuzz.token_set_ratio, score_cutoff=70)
      if x is not None:
        matches.append(x[0])
        ratio.append(x[1])
    ...
    df = pd.DataFrame(
      list(zip(left, matches, ratio)), columns=["original", "match", "ratio"]
    )
...
def dateToUnix(strDate):
  try:
    ...
    unix = int(
      dt(date.year, date.month, date.day, 0, 0)
...
def rgbFloat(mag, cmin, cmax):
  ...
  r = min((max((4 * (x - 0.25), 0.0)), 1.0))
  g = min((max((4 * np.fabs(x - 0.5) - 1.0, 0.0)), 1.0))
  b = min((max((4 * (0.75 - x), 0.0)), 1.0))
...
def insertNeo4jData(conn, query, rows, batchSize=10000):
  ...
  conn.query(
    "CREATE CONSTRAINT stock IF NOT EXISTS ON (s:security:stock) ASSERT s.id IS UNIQUE"
  )
  ...
  while batch * batchSize < len(rows):
    res = conn.query(
      query,
      parameters={
        "rows": rows[batch * batchSize : (batch + 1) * batchSize].to_dict("records")
      },
    )
    total += res[0]["total"]
    batch += 1
...
# %% Option trading
class BlackScholes:
  def __init__(self, S, K, T, sigma, r=0.01, q=0):
    self.S = S  # Spot price
  ...
  @property
  def params(self):
    return {"S": self.S, "K": self.K, "r": self.r, "sigma": self.sigma, "q": self.q}

  def d1(self):
    return (
      np.log(self.S / self.K) + (self.r - self.q + self.sigma**2 / 2) * self.T
  ...
  def _callPrice(self):
    return self.S * np.exp(-self.q * self.T) * self.N(self.d1()) - self.K * np.exp(
      -self.r * self.T
    ) * self.N(self.d2())

  def _putPrice(self):
    return self.K * np.exp(-self.r * self.T) * self.N(-self.d2()) - self.S * np.exp(
      -self.q * self.T
    ) * self.N(-self.d1())
...
def ndaqOptionData(ticker):
  ...
  def g(row, spot, rfr):
    t = (row["expiry"] - dt.now()).days / 365
    return BlackScholes(spot, row["strike"], t, row["impliedVolatility"], rfr).delta(
      row["optionType"]
    )
...
def ADI(ohlcv):
  n = ((ohlcv["close"] - ohlcv["low"]) - (ohlcv["high"] - ohlcv["close"])) / (
...
# Ease of movement
def EMV(ohlcv, p):
  pm = (ohlcv["high"] + ohlcv["low"]) / 2 - (
    ohlcv["high"].shift(1) + ohlcv["low"].shift(1)
  ) / 2
...
def MFI(ohlcv, p):
  ...
  mr = (
    pmf.rolling(window=p, min_periods=0).sum()
    / nmf.rolling(window=p, min_periods=0).sum()
  )
...
def SO(ohlc, p=20):
  k = (ohlc["close"] - ohlc["low"].rolling(p).min()) / (
    ohlc["high"].rolling(p).max() - ohlc["low"].rolling(p).min()
  )
  d = zlema(k, 5)
  dSlow = zlema(d, 5)
...
def pivotPoint(x, isMin):
  ...
  pivMax = [
    ...
    and (
      mom[i] == 0  # Slope is 0
      or (
        i != len(mom) - 1  # Check next day
        and (mom[i] > 0 and mom[i + 1] < 0 and clarr[i] >= clarr[i + 1])
        or i != 0  # Check prior day
        and (mom[i - 1] > 0 and mom[i] < 0 and clarr[i - 1] < clarr[i])
      )
    )
  ]
  ...
  pivMin = [
    ...
    and (
      ...
      or (
        i != len(mom) - 1  # Check next day
        and (
          mom[i] < 0 and mom[i + 1] > 0 and clarr[i] <= clarr[i + 1]  # Minima
        )
        or i != 0  # Check prior day
        and (mom[i - 1] < 0 and mom[i] > 0 and clarr[i - 1] > clarr[i])
      )
...
def gbm(close, steps, arch=True):
  ...
  simDates = pd.bdate_range(
    start=close.index[-1] + relativedelta(days=1), periods=steps
  )
...
def mlReg(data, fcPeriod, algo, rec=True, valSplit=0.1):
  ...
  )  # MinMaxScaler(feature_range=(0, 1)) scaler.inverse_transform()
  ...
  if rec:
    ...
    for i in range(1, fcPeriod):
      ...
      data.loc[fcDate[0], [c for c in fcFeat.columns]] = fcFeat.loc[
        fcDate[0], [c for c in fcFeat.columns]
      ]
    ...
    fcDate = pd.bdate_range(
      start=data.index[-1] + relativedelta(days=1), periods=fcPeriod
    )
...
# LSTM model creator
def lstmMdl(inShp, hl=(128, 128), dl=(32, 1), act="elu", keInit="he_uniform"):
  """
  Parameters
  ----------
  inShp : tuple
...
# LSTM
def lstm(
  data, fcPeriod, seqLen=60, valSplit=0.1, batch=100, epoch=10, trend=False, valid=False
):
  shft = 1
  ...
  model.compile(
    loss="mse",  #  mean_squared_error/sparse_categorical_crossentropy
    optimizer=opt,
    metrics=["mse"],
  )
  ...
  if recurs:
    ...
    for s in range(1, sims):
      ...
      for i in range(p):
        ...
        temp.loc[fcDate[0], [c for c in featSims.columns]] = featSims.loc[
          fcDate[0], [c for c in featSims.columns]
        ]
...
def hlt(x, fcPeriod, alfa=0.8, beta=0.2):
  model = Holt(x.to_numpy(), exponential=True, damped=False).fit(
    smoothing_level=alfa, smoothing_slope=beta, optimized=True
  )
...
def hmm(x, roc=True, n=2):
  ...
  model = GaussianHMM(n_components=n, covariance_type="full", n_iter=1000).fit(x)
...
def fbProphet(x, fcPeriod, bc=True):
  ...
  if bc:
    fc[["yhat", "yhat_upper", "yhat_lower"]] = fc[
      ["yhat", "yhat_upper", "yhat_lower"]
    ].apply(lambda x: inv_boxcox(x, lamb))
...
class Portfolio:
  ...
  def optimize(self, problem, riskRatio):
    ...
    cols = np.concatenate((np.array(["Return", "Risk", "RAR"]), self.tickers))
...
def tickersToDb(df, src, db):
  if db == "sqlite":
    engine = sqla.create_engine(
      r"sqlite:///C:\Users\danfy\OneDrive\FinAnly\data\ticker.db"
    )
    ...
    for r in tqdm(records):
      ...
      query = {"$and": [{"type": r["type"]}, {"ticker.trim": r["tickerTrim"]}]}
      if r["type"] == "stock":
        query["$and"].append(
          {"exchange.morningstarMic": mics["morningstarMic"]},
        )
      ...
      if coll.count_documents(query) == 0:  # Fuzzy name match
        ...
        if coll.count_documents(query) == 0:
          ...
          stored = pd.DataFrame.from_dict(coll.find(query, {"name": 1}))
          ...
          if fuzzName is not None:
            query = {"$and": [{"type": r["type"]}, {"name": fuzzName[0]}]}
            if r["type"] == "stock":
              query["$and"].append({"exchange.morningstarMic": mics["morningstarMic"]})
            ...
            operations.append(
              UpdateOne(query, updt, upsert=True)
            )  # coll.update_one(query, updt)
          else:
            insertNew = True
        ...
        if insertNew:  # Insert new document
          query = {
            "type": r["type"],
            "ticker": {f"{src}": r["ticker"], "trim": r["tickerTrim"]},
            f"name.{src}": r["name"],
            "exchange": {"mic": mics["mic"], "morningstar": mics["morningstarMic"]},
          }
          if src in ["barrons", "investing"]:
        ...
        updt = {
          "$set": {f"ticker.{src}": r["ticker"], "exchange.mic": mics["mic"]},
          "$setOnInsert": {
            "ticker.trim": r["tickerTrim"],
            "name": r["name"],
          },
        ...
        operations.append(
          UpdateOne(query, updt, upsert=True)
        )  # coll.update_one(query, updt, upsert=True)
...
def yfOhlc(ticker, startDate, store=True):
  ...
  if store:
    engine = sqla.create_engine(
      r"sqlite:///C:\Users\danfy\OneDrive\FinAnly\data\tickerData.db"
    )
    ...
    for i, t in enumerate(ticker):
      if not engine.dialect.has_table(engine, t):
        ...
        parse["date"] = pd.to_datetime(parse["date"], format="%Y-%m-%d %H:%M:%S.%f")
...
def mTwoPerCapita():
  # US population
  url = (
    "https://api.census.gov/data/1990/pep/int_natrespop" "?get=YEAR,MONTH,TOT_POP"
  )  # 1990-2000
  ...
  url = [
    ...
    (
      "https://api.census.gov/data/2019/pep/natmonthly" "?get=POP,MONTHLY_DESC&for=us:1"
    ),
  ]
...
def macroFeatures(startDate, roc=False):
  ...
  gdp = quandlData(ticker="FRED/GDP", startDate=startDate, valName="gpd")
  ...
  for q in quandlQuery:
    val = q.split("/")[-1]
    features = quandlData(
      ticker=q, startDate=startDate, valName=val, df=features, merge=True
    )
...
def techFeatures(ohlc, rec=True):
  ...
  ohlc["std"] = (
    ohlc["close"].rolling(window=20, min_periods=0).std()
  )  # Standard deviation
...
def getNorgesBankData(what, startDate, currency, bondType, df, merge=False):
  ...
  data = pd.read_csv(
    buff, delimiter=",", index_col=0, parse_dates=True, date_parser=dateparse
  )
  data.index.names = ["date"]
...
def bitcoinData():
  ...
  for i, k in enumerate(metrics):
    ...
    scrap = []
    for j in parse:
      scrap.append({"date": dt.fromtimestamp(i[0]), metrics[k]: i[1]})
...
def worldBankData(tickers, startYear=""):
  ...
  def parser(tickers, startYear):
    ...
    for i, k in enumerate(tickers):
      tmp = wb.get_series(k, date=period, id_or_value="id", simplify_index=True)
  ...
  if not engine.dialect.has_table(engine, "wb"):
    ...
    parse.index = pd.MultiIndex.from_frame(
      parse[["country", "year"]], names=["name", "date"]
    )
