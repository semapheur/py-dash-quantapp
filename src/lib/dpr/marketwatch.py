...
class Ticker:
    ...
    def financials(self):
        ...
        if 'stInv' not in cols:
            itm = [
                'trdAcntSecInv', 'bnkRsrvSldSecPrchInv', 'othSecInv',
                'prfEqtInv', 'cmnEqtInv'
            ]
            df['stInv'] = 0
            for i in itm:
        ...
        if 'totCrtAst' not in cols:
            ...
            itm = [
                'acnRcv', 'intRcv', 'trdAcntSet', 'othSec', 'cnsmLoan',
                'isrcPrmRcv'
            ]
            for i in itm:
                if i in cols:
    ...
    def ohlcv(self, startDate='', period='all'):
        ...
        if period == 'all':
            ...
            timeArg = (
                '"TimeFrame":"",'
                f'"StartDate":{startStamp},"EndDate":{endStamp}')       
        ...
        df['volume'] = volume
        df.index = pd.to_datetime(ix, unit='ms').floor('D') # Convert index from unix to date
...
def getTickers():
    ...
    noXchg = {
        ...
        ('ConnectOne Bancorp Inc. Dep. Pfd. (Rep. 1/40th Non-Cum. Perp. Pfd. Series A)', 'CNOBP'): 'XNAS',
    ...
    mask = (
            df['ticker'].str.contains(r'\.W(I|T)[ABCDRS]?$', regex=True) | 
            (df['exchange'] == 'IPO') 
        )
    df = df[~mask]
