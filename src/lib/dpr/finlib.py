...
FINITEMS = {
  "incomeStatement": {
    "rvn": {
      ...
      "children": {
        "saleRvn": {
          ...
          "sign": 1,
          "children": {
            "domSaleRvn": {"label": "Domestic Sales Revenue", "sign": 1},
            "frgnSaleRvn": {"label": "Foreign Sales Revenue", "sign": 1},
          },
        },
        "finRvn": {"label": "Financing Revenue", "sign": 1},
        "othRvn": {"label": "Other Revenue", "sign": 1},
      },
    },
    "rvnEx": {
      ...
      "abbreviation": "COGS",
      "children": {
        "saleRvnEx": {"label": "Cost of Sales Revenue", "sign": 1},
        "finRvnEx": {"label": "Cost of Financing Revenue", "sign": 1},
        "othRvnEx": {"label": "Cost of Other Revenue", "sign": 1},
      },
    },
    "grsPrft": {"label": "Gross Profit", "formula": "rvn - rvnEx"},
    "opEx": {"label": "Operating Expense", "formula": "rdEx + sgaEx + dda + oth"},
    "rdEx": {
    ...
    "dda": {"label": "Depreciation, Depletion & Amortization", "abbreviation": "DDA"},
    "othOpEx": {
      "label": "Other Operating Expense",
    },
    "opInc": {"label": "Operating Income/Loss", "formula": "grsPrft -opEx"},
    "noOpInc": {
    ...
    "impr": {"label": "Impairment", "formula": "imprTgbAst +imprItgbAst +imprGw"},
    "ebt": {
    ...
    "netIncCntOp": {"label": "Net Income from Continuing Operations"},
    "netIncDscOp": {"label": "Net Income from Discontinued Operations"},
    "netInc": {"label": "Net Income", "formula": "ebt - taxEx"},
    "sh": {"label": " Weighted Average Shares Outstanding (basic)"},
    "shDil": {"label": "Weighted Average Shares Outstanding (diluted)"},
    "eps": {
  ...
  "assetSheet": {
    "cce": {"label": "Cash & Cash Equivalents", "abbreviation": "CCE"},
    "stInv": {"label": "Short-term Investments"},
    "cceStInv": {
      "label": "Cash, Cash Equivalents & Short-term Investments",
      "formula": "cce +stInv",
    },
    "acntRcv": {"label": "Account Receivable"},
    "ivty": {"label": "Inventory", "formula": "ivtyRaw +ivtyWip +ivtyFg +othIvty"},
    "othCrtAst": {
    ...
    "totAst": {"label": "Total Assets", "formula": "totCrtAst +totNoCrtAst"},
  },
  "liabilitySheet": {},
  "equitySheet": {},
  "cashFlowStatement": {
    "opCf": {"label": "Operational Cash Flow"},
    "invCf": {"label": "Investing Cash Flow"},
    "finCf": {"label": "Financial Cash Flow"},
    "freeCf": {"label": "Free Cash Flow"},
    "freeCfFirm": {"label": "Free Cash Flow to Firm"},
  },
}
...
def finItemRenameDict(src) -> dict:
  ...
  res = {k: v for k, v in zip(df["sourceLabel"], df["itemValue"])}
  return res
...
def getFinData(
  parser, dbName, tblName, tickerId="", ix=["id", "date"], updateInterval=0
):
  ...
  if tickerId:
    ...
    if not insp.has_table(tblName):
      ...
      df = pd.read_sql(
        query, con=engine, index_col=ix, parse_dates={"date": {"format": "%Y-%m-%d"}}
      )
      ...
      if relativedelta(dt.now(), lastDate).days > updateInterval:
        ...
        if dfNew is not None:
          ...
          df.reset_index().to_sql(
            tickerId, con=engine, if_exists="replace", index=False
          )
...
def wacc(
  df, quoteParser, mktParser, rskFreeParser, tickerId="", betaPeriod=1, dbtMaturity=5
):  # yahoo: ^TNX/'^TYX; fred: DSG10
  ...
  if df["mktCap"].isnull().all():
    df["capCls"] = df["totAst"].apply(lambda x: "small" if x < 2e9 else "large")

  else:
    df["capCls"] = df["mktCap"].apply(lambda x: "small" if x < 2e9 else "large")

  df["crdtSprd"] = df.apply(lambda r: creditRating(r["intCvg"], r["capCls"]), axis=1)

  beta = {"beta": [], "mktRet": [], "rfr": []}
  ...
  for i in range(len(ixDates)):
    if i == 0:
      mask = (dfReturns.index >= min(ixDates[i], dfReturns.index.min())) & (
  ...
  df = (
    df.reset_index().merge(dfBeta, on="date", how="left").set_index(["date", "period"])
  )

  df["betaLvr"] = df["beta"] * (1 + (1 - df["taxRate"]) * df["totDbt"] / df["totEqt"])

  # Cost of equity
  df["cstEqt"] = df["rfr"] + df["betaLvr"] * (df["mktRet"] - df["rfr"])
  # Cost of debt
  ...
  df["wacc"] = df["cstEqt"] * df["mktCap"] / (df["mktCap"] + df["mktValDbt"]) + df[
    "cstDbt"
  ] * (1 - df["taxRate"]) * df["totDbt"] / (df["mktCap"] + df["mktValDbt"])
...
def financialRatios(df):
  ...
  if "roe" not in df.columns:
    df["roe"] = df["netInc"] / df["totEqt"].rolling(2, min_periods=0).mean()

  # Return on tangible common equity
  if "tgbEqt" in df.columns:
    df["rote"] = df["netInc"] / df["tgbEqt"].rolling(2, min_periods=0).mean()

  # Return on assets
  if "roa" not in df.columns:
    df["roa"] = df["netInc"] / df["totAst"].rolling(2, min_periods=0).mean()
  ...
  df["roic"] = df["netInc"] / (df["totEqt"] + df["totDbt"])
  ...
  df["cfDbtRatio"] = df["opCf"] / df["totDbt"].rolling(2, min_periods=0).mean()

  # Gross profit margin
  if "gpm" not in df.columns:
    df["gpm"] = (df["rvn"] - df["rvnEx"]) / df["rvn"]
  ...
  df["dpo"] = 365 * (df["acntPbl"].rolling(2, min_periods=0).mean() / df["opEx"])
...
def priceMultiples(df):
  # Data frequency
  span = relativedelta(
    df.index.get_level_values("date").max(), df.index.get_level_values("date").min()
  ).years
  freq = int(np.ceil(len(df) / span)) if span != 0 else 1

  # Price ratios
  if "shQuote" not in df.columns and df["shDil"].isna().all:
    raise ValueError
  ...
  df["pncavRatio"] = df["mktCap"] / (
    df["totCrtAst"] - df["totLbt"] - df["prfEqt"].fillna(0)
  )

  # Price to Net Net Working Capital
  df["pnnwcRatio"] = df["mktCap"] / (
    df["cceStInv"] + 0.75 * df["acntRcv"] + 0.5 * df["ivty"] - df["totLbt"]
  )
...
def ggm(df, fcPer=20, ltGrowth=0.03):
  ...
  ev = d * ((1 + meanGrowth) + h * (ltGrowth - meanGrowth)) / (wacc - meanGrowth)
...
def zScore(df):
  ...
  df["zScore"] = (
    1.2 * wc / ta + 1.4 * re / ta + 3.3 * ebitda / ta + s / ta + 0.6 * e / tl
  )
...
def mScore(df):
  ...
  # Asset Quality Index
  aqi = 1 - (df["wrkCap"] + df["ppe"] + df["ltInv"]) / df["totAst"]
...
def getFundamentals(
  finParser, quoteParser, mktParser, rskFreeParser, tickerId="", betaPeriod=5
):
...
def mergeTickers(asset="stock"):
  ...
  for src in fuzzyXchg.keys():
    ...
    df = df.merge(dfTemp, on=["tickerTrim", "morningstarMic"], how="outer")
  ...
  for src, xchg in fuzzyXchg.items():
    for x in tqdm(xchg):
      ...
      mask = df[f"{src}Name"].isin(
        dfMatch[f"{src}Name"].dropna()
      ) & df.index.get_level_values("morningstarMic").isin(x[-1])
      df = df[~mask]
