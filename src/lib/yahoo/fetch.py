...
def crumb_cookies() -> tuple[str, httpx.Cookies]:
  ...
  crumb_store = soup.xpath('//script[contains(text(), "CrumbStore")]/text()').get()
  if crumb_store is None:
    raise ValueError("Could not get crumb store!")

  crumb = re.search(r'"CrumbStore":{"crumb":"(.+?)"}', crumb_store)
  if crumb is None:
    raise ValueError("Could not get crumb!")

  return crumb.group(1), response.cookies
