...
from lib.utils.dataframe import df_time_difference_pandas
...
def trailing_twelve_months(financials: DataFrame) -> DataFrame:
  ...
  ttm["month_difference"] = df_time_difference_pandas(
...
def get_days(
  ...
  for s in slices:
    ...
    days_["month_difference"] = df_time_difference_pandas(
      cast(pd.DatetimeIndex, days_.index.get_level_values("date")), 30, "D"
    )
    mask_ = days_["month_difference"] == s[2]
    days_.loc[mask_, "days"] = df_time_difference_pandas(
...
def applier(
  ...
  for ix in slices:
    ...
    month_diff = pd.Series(df_time_difference_pandas(dates, 30, "D"), index=s_.index)
