...
class Ticker:
    ...
    apiKey = 'fwJro7kqt0rbxXfaMU2SvL9Gw5aOIQFU' 
    ...
    def financial(self, startYear='2000'):
        ...
        for i, statement in enumerate(statements):
            ...
            if statement == 'pl' or statement == 'cf':               
                ...
                temp.rename(columns={'Minority Interest': 'accruedMinInterest'},
                            inplace=True)
            
            elif statement == 'derived':
                ttmCols = ['EBITDA', 'Free Cash Flow', 'Earnings Per Share, Basic', 'Earnings Per Share, Diluted', 
                          'Sales Per Share', 'Free Cash Flow Per Share', 'Dividends Per Share']
        ...
        delCol = ['SimFinId', 'Ticker', 'Fiscal Period', 'Fiscal Year', 'TTM', 
                  'Value Check', 'Net Income/Starting Line',  
        ...
        df['wrkCap'] = (
            (df['cce'] + df['stInv'] + 0.75 * df['rcv'] +
             0.5 * df['invnt']).rolling(2, min_periods=0).mean() - 
        ...
        # Drop nan columns
        posts = [
            'sgaEx', 'rdEp', 'cce', 'stInv', 'ivty', 'rcv', 
            'totCrtAst', 'ltInv', 'gw', 'itgbAst', 'stDbt', 
            'ltCapLeas', 'prfEqt', 'dvd', 'wrkCap']
        for p in posts:
            df[p] = df[p].fillna(0)
...
def getTickers():
    ...
    df.rename(columns={'SimFinId': 'simfinId', 'Ticker': 'ticker'}, 
        inplace=True)
...
def bulkFinancials(market='us'):
    ...
    dfs = []
    dropCols = ['SimFinId', 'Fiscal Year', 'Fiscal Period', 'Publish Date', 'Restated Date']
    for sheet in ['income', 'balance', 'cashflow']:
        
        dfPeriod = []
        for p in ['annual', 'quarterly']:
        
            temp = sf.load(dataset=sheet, variant=p, market=market,
                    index=[TICKER, REPORT_DATE],
                    parse_dates=[REPORT_DATE]) # refresh_days=30
