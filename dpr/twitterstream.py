...
# API-key: 'FpVpgILWD8UhzzvuIMxDOTYs3'
# API secret key: 'oIXr0dskMhMCFerzWgb6mmqzzOVIyWHlGBaIk4Yg8reM1DiOzm'
# Bearer token: 'AAAAAAAAAAAAAAAAAAAAAICiKQEAAAAAsjdXZ7jcpG8%2FKrGG9CT3al5EMIY%3DK0iNVogEvK8lnDNhFZ2YXQB49EChK3UHZO003xthnDrk0QmTQA'
# Access token: '1257725628020936707-DydLSUTCLjJqStSUX3vWHwExRU4ptK'
# Access token secret: 'HtQ9yeZibBhY0T9PcEXxDxcWRUZ4yRRqs1OnYkC0Eilvm'
...
class CacheSqlite:
    ...
    def cleanCache(self):
        ...
        for table in self.tables:
            self.cursor.execute(f'DELETE FROM {table} WHERE expires < ?', 
                                (int(time.time()),))
    ...
    def get(self, pool, key):
        ...
        # Get data from cache
        result = self.cursor.execute(f'SELECT value FROM {pool} WHERE key = ?', 
                                     (key,)).fetchone()
    ...
    def set(self, pool, key, value, ttl=0):
        ...
        self.cursor.execute(f'REPLACE INTO {pool} VALUES (?, ?, ?)', 
                            (key, pickle.dumps(value), 
                             int(time.time() + ttl) 
                             if ttl > 0 and ttl <= 2592000 else ttl))
    ...
    def saveDb(self):
        ...
        with self.lock:
            if len(self.data):
                ...
                try:
                    self.c.executemany("INSERT INTO sentiment (unix, tweet, sentiment) VALUES (?, ?, ?)", self.data)
                except:
                    pass
...
def generateTrending(lock, conn, c, splitRegex, blacklistCounter):
    ...
    try:
        ...
        # Make tokens
        tokens = splitRegex.split(' '.join(list(itertools.chain.from_iterable(df['nouns'].values.tolist()))).lower())
