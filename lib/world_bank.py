...
def fetch_wdi(end_year=0, start_year=1960) -> pd.DataFrame:
  ...
  df = wb.data.DataFrame(list(SERIES.keys()), time=range(start_year, end_year))
...
def load_wdi(cols: str | list[str] = '*', years=0, delta=2) -> pd.DataFrame:
  ...
  if df.empty:
    df = fetch_wdi()
    return cast(pd.DataFrame, df.unstack(level=1))

  df = cast(pd.DataFrame, df.unstack(level=1))
