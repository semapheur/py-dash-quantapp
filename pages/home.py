from dash import callback, ctx, html, no_update, register_page, Input, Output, State, MATCH
...
layout = html.Main(className=main_style, children=[
  ...
  html.Div(className='h-full flex flex-col rounded shadow', children=[
    html.Form(className='grid grid-cols-[2fr_1fr_auto] gap-2 px-2 pt-2', children=[
...
@callback(
  Output(QuoteGraphAIO._id('home'), 'figure'),
  Input(QuoteStoreAIO._id('home'), 'data'),
  Input(QuoteGraphAIO._id('home'), 'relayoutData'),
  Input(QuoteGraphTypeAIO._id('home'), 'value'),
  Input(QuoteDatePickerAIO._id('home'), 'start_date'),
  Input(QuoteDatePickerAIO._id('home'), 'end_date'),
  State(QuoteGraphAIO._id('home'), 'figure')
)
